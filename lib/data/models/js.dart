...
class JSList extends TaskList {
  ...
  JSList({List<JS> jsList}) : _jsList = jsList;
...
class JS extends SubTask {
  ...
  get chunks {
    List<String> value = [];
    for (String chunk in _chunks) value.add(chunk);
    value.shuffle();
    return value;
  }
  ...
  factory JS.fromJson(
    ...
    for (String chunk in question['chunks']) chunkList.add(chunk);
  ...
  void debugMessage() {
    ...
    if (sentence != null) print("Sentence: " + sentence);
  ...
  String concatenateChunks(List<String> tokens) {
    String result = "";
    for (String chunk in tokens) result = result + " " + chunk;
    return result.trim();
  }
