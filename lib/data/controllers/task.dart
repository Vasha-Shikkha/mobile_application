...
class TaskController {
  ...
  Future<List<TaskList>> getTaskList(
    ...
    int count =
        await taskDatabaseHelper.getCount2(topicId, level, limit, offset);
    // print(count);
    ...
    if (count == 0) {
      List<TaskList> tasks =
          await TaskRest().getTaskList(token, topicId, level, limit, offset);
      print("Geroro");
      for (TaskList task in tasks) {
        ...
        for (int i = 0; i < task.taskList.length; i++) {
          ...
          // print("Task to DB");
          await taskDatabaseHelper
              .insertTask(obj.toTask()); //it will simply replace the same task
          print("SubTask to DB");
    ...
    List<TopicTask> taskDetails = await taskDatabaseHelper
        .getTasks(topicId, level, limit: limit, offset: offset);
    ...
    for (TopicTask taskDetail in taskDetails) {
      ...
      if (taskName == 'Picture to Word') {
        ...
        List<PW> pwSubtasks = await pwDatabaseHelper
            .getPWs(topicId, level, taskDetail, limit: limit, offset: offset);
        print("Mahoyo");
        result.add(new TaskList(taskList: pwSubtasks, taskName: taskName));
      } else if (taskName == 'Sentence Matching') {
        print("Kamen Rider");
        List<SM> smSubtasks = await smDatabaseHelper
            .getSMs(topicId, level, taskDetail, limit: limit, offset: offset);
        result.add(new TaskList(taskList: smSubtasks, taskName: taskName));
      } else if (taskName == 'Jumbled Sentence') {
        print("Fate/Stay Night");
        List<JS> jsSubtasks = await jsDatabaseHelper
            .getJSs(topicId, level, taskDetail, limit: limit, offset: offset);
        result.add(new TaskList(taskList: jsSubtasks, taskName: taskName));
      } else if (taskName == 'Fill in the Blanks') {
        print("Tales of Berseria");
        List<FB> fbSubtasks = await fbDatabaseHelper
            .getFBs(topicId, level, taskDetail, limit: limit, offset: offset);
        result.add(new TaskList(taskList: fbSubtasks, taskName: taskName));
      } else if (taskName == 'MCQ') {
        print("Psycho Pass");
        List<MCQ> mcqSubtasks = await mcqDatabaseHelper
            .getMCQs(topicId, level, taskDetail, limit: limit, offset: offset);
        result.add(new TaskList(taskList: mcqSubtasks, taskName: taskName));
      } else if (taskName == 'Error in Sentence') {
        print("Eiyuuden");
        List<Error> errorSubtasks = await errorDatabaseHelper.getErrors(
            topicId, level, taskDetail,
